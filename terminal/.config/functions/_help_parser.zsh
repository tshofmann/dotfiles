...
_help_parse_aliases() {
    ...
    while IFS= read -r line; do
        ...
        # Special handling for guard clauses (if command -v ... or if ! command -v ...)
...
_help_parse_functions() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^#\ ([^=].+)$ ]]; then
            local comment="${match[1]}"
            # Skip section headers and documentation links
            if [[ ! "$comment" =~ ^(Zweck|Pfad|Docs|Hinweis|Guard|Basis|Essenzielle|Erweiterte|Interaktive|Update|Baumansicht|Mit\ Git|Dateiansicht|Dateityp|Mac\ App|Voraussetzung|------------) ]]; then
                prev_comment="$comment"
            fi
...
_help_get_tool_version() {
    ...
    case "$tool" in
        eza)
            # eza outputs: "eza - A modern...  v0.18.0"
            version=$("$tool" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | tr -d 'v' | head -1)
            ;;
        bat|fd|fzf|rg|ripgrep|starship|zoxide|gh|btop)
